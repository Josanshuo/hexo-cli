...
function initConsole(args) {
  ...
  var starterGitRepoURL = args.starter_git || GIT_REPO_URL;
  ...
  log.info('Cloning hexo-starter', starterGitRepoURL);

  if (args.clone) {
    promise = spawn('git', ['clone', '--recursive', starterGitRepoURL, target], {
      stdio: 'inherit'
    });
  } else {
